...
def extract_level(text):
    ...
    # This lists will hold words in that level

    with open('a1_words_all.txt') as f:
        for line in f.readlines():
            a1_words.append(line.split("=>")[0][:-1])

    for i in a1_words:
        if len(i) <= 2:
            a1_words.remove(i)

    with open('a2_words_all.txt') as f:
        for line in f.readlines():
            a2_words.append(line.split("=>")[0][:-1])

    for i in a2_words:
        if len(i) <= 2:
            a2_words.remove(i)

    with open('b1_words_all.txt') as f:
        for line in f.readlines():
            b1_words.append(line.split("=>")[0][:-1])

    for i in b1_words:
        if len(i) <= 2:
            b1_words.remove(i)

    with open('b2c1_words_all.txt') as f:
        for line in f.readlines():
            b2c1_words.append(line.split("=>")[0][:-1])
    ...
    # This dict will hold score of that level

    product = 1

    for i in range(number_of_most_element):

        if (frequency(text)[i]) in a1_words:
            my_dict["A1"] += product / i

        elif (frequency(text)[i]) in a2_words:
            my_dict["A2"] += product / i


        elif (frequency(text)[i]) in b1_words:
            my_dict["B1"] += product / i


        elif (frequency(text)[i]) in b2c1_words:
            my_dict["B2C1"] += product / i
